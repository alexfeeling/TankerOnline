package ui 
{
	...
	public class ControllUI extends Sprite 
	{
		...
		private var _turnLeftBtn:Sprite;
		private var _turnRightBtn:Sprite;
		...
		private function init():void {
			_turnLeftBtn = new Sprite();
			_turnLeftBtn.graphics.beginFill(0xffcc00, 0.5);
			_turnLeftBtn.graphics.drawRect(0, 0, 150, 150);
			_turnLeftBtn.graphics.endFill();
			_turnLeftBtn.x = 0;
			_turnLeftBtn.y = World.STAGE_HEIGHT - 150;
			this.addChild(_turnLeftBtn);
			_turnLeftBtn.addEventListener(TouchEvent.TOUCH_BEGIN, function (evt:TouchEvent):void { 
				Commander.sendOrder(OrderConst.KEY_LEFT_PRESSING);
			} );
			_turnLeftBtn.addEventListener(TouchEvent.TOUCH_ROLL_OUT, function (evt:TouchEvent):void {
				Commander.sendOrder(OrderConst.KEY_LEFT_RELEASE);
			});
			_turnLeftBtn.addEventListener(TouchEvent.TOUCH_END, function (evt:TouchEvent):void { 
				Commander.sendOrder(OrderConst.KEY_LEFT_RELEASE);
			} );
			
			_turnRightBtn = new Sprite();
			_turnRightBtn.graphics.beginFill(0x0000ff, 0.5);
			_turnRightBtn.graphics.drawRect(0, 0, 150, 150);
			_turnRightBtn.graphics.endFill();
			_turnRightBtn.x = 150;
			_turnRightBtn.y = World.STAGE_HEIGHT - 150;
			this.addChild(_turnRightBtn);
			_turnRightBtn.addEventListener(TouchEvent.TOUCH_BEGIN, function(evt:TouchEvent):void { 
				Commander.sendOrder(OrderConst.KEY_RIGHT_PRESSING);
			});
			_turnRightBtn.addEventListener(TouchEvent.TOUCH_ROLL_OUT, function(evt:TouchEvent):void {
				Commander.sendOrder(OrderConst.KEY_RIGHT_RELEASE);
			});
			_turnRightBtn.addEventListener(TouchEvent.TOUCH_END, function(evt:TouchEvent):void { 
				Commander.sendOrder(OrderConst.KEY_RIGHT_RELEASE);
			} );
		}
